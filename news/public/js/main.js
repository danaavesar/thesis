...
app.main = (function() {
	...
	var checkSimilarities = function(query){
		...
		for(var headline in queryMatches){ //for every headline
	
			var wordsInHeadline = headline.split(" "); 

			for(var headline2 in queryMatches){ //search through all the other headlines
				queryMatches[headline2].matchingHeadline = [];
				if(headline2 != headline){ //except the one you are on
					//split the words of that headline in an array
					for(var i=0; i<wordsInHeadline.length; i++){ //for all the words in that headline
						if(wordsInHeadline[i] != query && wordsInHeadline[i] != ""){ //dont do it for the query word
							if(headline2.indexOf(wordsInHeadline[i]) != -1 ){
								...
								queryMatches[headline2].matchingHeadline.push(wordsInHeadline[i] + " " + headline);
