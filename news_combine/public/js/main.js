...
app.main = (function() {
	...
	var findSimilarHeadlines = function(query){
		for(var i=0; i <headlineArray.length; i++){
			...
			console.log(article);
			headlineArray.splice(0, 1);

			console.log(article);
			var matchingKeyword = lookForKeyword(i,keyword);
		}
		
		matchingKeyWords.push(matchingKeyword);

	}

	var lookForKeyword = function(i,keyword){
		for(var i=0; i<headlineArray.length; i++){
			for(var k = 0; k<headlineArray[i].keywords.length; k++){
				if(headlineArray[i].keywords[k].match(keyword)){
					return headlineArray[i].keywords[k];
				}
			}
		}
	}
	...
	// 	for(var i=0; i<headlineArray.length; i++){ //headline being compared too
	// 		if(headlineArray[i].keywords != null){ //
				
	// 			/*1. loop through all the other headlines */	
	// 			for(var j=0; j< headlineArray.length; j++){ //headline being compared with
	// 				var commonKeywordArray = [];
	// 				var common = false;
	// 				var headlineKeywords = headlineArray[j].keywords
	// 			 	///skip the i headline and dont do this if the headline doesnt have keywords
	// 			 	//
	// 			 	if(j != i && headlineKeywords != null){
	// 				 	/*loop through the keywords of the headline*/
	// 				 	for(var k=0; k <headlineKeywords.length; k++){
	// 				 		var keyword = headlineKeywords[k].text;
	// 				 		loop through each keyword of the main headline i
	// 				 		for(var s = 0; s< headlineArray[i].keywords.length; s++){
	// 				 			var mainKeyword = headlineArray[i].keywords[s].text;
	// 				 			/*check to see if the keywords match*/
	// 				 			if(keyword.indexOf(mainKeyword) > -1 || mainKeyword.indexOf(keyword) > -1 && keyword.indexOf(query) < 0  ){
	...
	// 				 	if(common){
	// 				 		makeRelatedArray(headlineArray[i], headlineArray[j], commonKeywordArray);
	// 				 	}else{
	...
	// var makeRelatedArray = function(commonHeadlineI, commonHeadlineJ, commonKeywordArray){
	// 	var thingToPush = {
	// 		'keywordsInCommon' : commonKeywordArray,
	// 		'headlines' : [commonHeadlineI, commonHeadlineJ]
	// 	}
	// 	var newKeywordMatch = false;
	// 	//if the array doesnt already have those keywords logged then push it in as a new keyword
	// 	if(relatedHeadlineArray.length > 0){
	// 		for(var i =0; i<relatedHeadlineArray.length;i++){ //for each object in related headline array
	// 			for(var j =0; j <relatedHeadlineArray[i].keywordsInCommon.length; j++){ //loop through the keywords in common
	// 				//loop through the new common key words
	// 				for(var f=0; f<commonKeywordArray; f++){
	// 					if(relatedHeadlineArray[i].keywordsInCommon[j] != commonKeywordArray[f]){
	// 						newKeywordMatch = true;
	// 					}
	// 				}
	// 			}
	// 		}
	// 	}else{ //if the array has nothign in it yet
	// 		relatedHeadlineArray.push(thingToPush);
	// 	}
